...
- `llm_translate.py`: translates extracted JSON through an OpenAI-compatible chat completions API. It preserves the JSON shape, fills `translated` fields, supports `--resume`, validates returned batches, and stops on partial or malformed model output.
...
- Translation progress is shown as a fixed `0` to `100%` bar based on total translatable entries in the full file, not only the entries translated in the current run.
